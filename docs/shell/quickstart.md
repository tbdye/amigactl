...
2. **Python client** is available on the host machine (Python 3.8+).
   Either run directly via the wrapper script (`client/amigactl.sh`) or
   install with pip:

       pip install -e client/

3. **Network connectivity** between host and Amiga. Verify with:

       amigactl --host 192.168.6.228 ping
...
- **CLI flag:** `--host 192.168.6.228`
- **Environment variable:** `AMIGACTL_HOST=192.168.6.228`
...
```
Connected to 192.168.6.228 (amigactld 0.8.0)
...
amiga@192.168.6.228:SYS:S> cd Work:
amiga@192.168.6.228:Work:>
```
...
```bash
amiga@192.168.6.228:SYS:> exec avail
...
```bash
amiga@192.168.6.228:SYS:> put /tmp/myprog Work:myprog
...
```bash
amiga@192.168.6.228:SYS:> help
```

Get detailed help for a specific command:

```bash
amiga@192.168.6.228:SYS:> help ls
```
